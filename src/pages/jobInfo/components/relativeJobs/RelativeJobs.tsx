...
import { fetchRelativeJobs } from '@/store/slices/job/job.slice';
import { Store } from '@/store';
import { Skeleton } from 'antd';
import { formatTimeAgo } from '@/utils/common/format_time_ago_function';

export default function RelativeJobs({ type_job_id }: { type_job_id: number[] | undefined }) {
    const dispatch = useDispatch();
    const jobStore = useSelector((store: Store) => store.jobStore)
    const [checkValue, setCheckValue] = useState(false)
    useEffect(() => {
        if (type_job_id && !checkValue) {
            setCheckValue(true)
        }
    }, [type_job_id])
    useEffect(() => {
        if (checkValue) {
            dispatch(fetchRelativeJobs({ typeJobIdArray: type_job_id || [] }) as any)
        }
    }, [checkValue])
